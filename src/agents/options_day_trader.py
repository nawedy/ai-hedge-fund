...
class OptionsDayTraderSignal(BaseModel):
    signal: Literal["bullish", "bearish", "neutral"]
    confidence: float
    reasoning: str

def options_day_trader_agent(state: AgentState):
    """
    Analyzes options data for day trading and generates trading signals.
    """
    data = state["data"]
    ...
    for ticker in tickers:
        progress.update_status("options_day_trader_agent", ticker, "Fetching options metrics")
        metrics = get_option_metrics(ticker, end_date)
        
        progress.update_status("options_day_trader_agent", ticker, "Gathering financial line items")
        ...
        progress.update_status("options_day_trader_agent", ticker, "Analyzing options data")
        ...
        progress.update_status("options_day_trader_agent", ticker, "Generating Options Day Trader analysis")
        ...
        progress.update_status("options_day_trader_agent", ticker, "Done")
    
    message = HumanMessage(
        content=json.dumps(options_analysis),
        name="options_day_trader_agent"
    )
    
    if state["metadata"]["show_reasoning"]:
        show_agent_reasoning(options_analysis, "Options Day Trader Agent")
    
    state["data"]["analyst_signals"]["options_day_trader_agent"] = options_analysis
...
def generate_options_output(
    ...
    template = ChatPromptTemplate.from_messages([
        (
            "system",
            """You are an Options Day Trader AI agent, making trading decisions based on options data.
        ...
        (
            "human",
            """Based on the following analysis, create an Options Day Trader signal.
    ...
    def create_default_options_day_trader_signal():
        return OptionsDayTraderSignal(
            signal="neutral",
            confidence=0.0,
    ...
    return call_llm(
        ...
        pydantic_model=OptionsDayTraderSignal, 
        agent_name="options_day_trader_agent", 
        default_factory=create_default_options_day_trader_signal,
    )
